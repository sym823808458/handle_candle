...
只返回单个JSON对象，无需额外解释。"""
    ...
    def _workflow_execute_citation_expansion(self):
        ...
        try:
            ...
            # 🔑 关键修复：直接调用完整的引用网络扩展方法
            # 这个方法会读取 self.latest_analysis_file 并使用高质量文章进行筛选
            self._execute_citation_expansion(urls)
            
            # 🔑 确保扩展后的URLs被传递给下一轮
            if hasattr(self, 'latest_expanded_urls_file') and os.path.exists(self.latest_expanded_urls_file):
                # 读取扩展后的URLs
                citation_urls = []
                with open(self.latest_expanded_urls_file, 'r', encoding='utf-8') as f:
                    for line in f:
                        url = line.strip()
                        if url and url.startswith('http'):
    ...
    def _execute_search(self, question):
        ...
        try:
            ...
            for i, query in enumerate(search_queries):
                ...
                results = search_bing_academic(query, page_count=page_count)
    ...
    def _execute_citation_expansion(self, urls):
        """执行引用网络扩展 - 优化版：只查询引用文献，并为每个文献获取完整摘要"""
        try:
            print("\n" + "="*80)
            print("🔗 开始网络工具栏引用扩展过程（优化版 - 完整摘要获取）")
            print("="*80)
            print(f"📊 输入统计:")
            print(f"   - 待处理URL数量: {len(urls)}")
            print(f"   - 筛选数量限制: {self.citation_count_var.get()}")
            print(f"   💡 策略: 只查询引用文献，为每个文献单独获取完整信息（包括摘要）")
            ...
            print(f"\n📋 步骤a: 处理初始URL列表")
                    # 🔧 优化：只处理前N个URL以加快速度
            max_urls_to_process = 10  # 🔑 关键：从所有URL减少到只处理前10个
            urls_to_process = urls[:max_urls_to_process]
            
            if len(urls) > max_urls_to_process:
                print(f"   ⚡ 速度优化：从 {len(urls)} 个URL中选择前 {max_urls_to_process} 个处理")
                print(f"      - 已选择: {len(urls_to_process)} 个URL")
                print(f"      - 跳过: {len(urls) - max_urls_to_process} 个URL")
            else:
                print(f"   ✅ URL数量适中，处理全部 {len(urls_to_process)} 个")
            
            # 步骤b: 对每个URL，提取DOI并查询引用文献
            print(f"\n🔍 步骤b: 从URLs提取DOI并查询引用网络（含完整摘要）")
            for i, url in enumerate(urls_to_process):  # 🔑 只处理选中的URL
                ...
                if doi:
                    ...
                    # 🔑 关键改进：使用增强版查询函数（会为每个引用文献获取完整信息）
                    print(f"   📚 查询引用文献（它引用了谁）...")
                    print(f"   🔍 将为每个引用文献获取完整信息（包括摘要、期刊等）...")
                    
                    references = self._query_references_with_full_info(doi)
                    
                    print(f"   📚 找到引用文献: {len(references)} 篇")
                    
                    # 统计有摘要的文献数量
                    with_abstract = sum(1 for ref in references if ref.get('abstract') and len(str(ref.get('abstract', '')).strip()) > 50)
                    print(f"   📝 其中有摘要: {with_abstract} 篇 ({with_abstract/len(references)*100:.1f}%)" if references else "   📝 其中有摘要: 0 篇")
                    
                    all_citation_data.extend(references)
                else:
                    print(f"   ❌ 未能提取DOI")
            
            # 步骤e: 合并并去重，优先处理有摘要的文献
            print(f"\n🔄 步骤e: 合并引用数据并去重（优先有摘要的文献）")
            ...
            # 统计有摘要的文献
            with_abstract_count = sum(1 for c in deduplicated_citations if c.get('abstract') and len(str(c.get('abstract', '')).strip()) > 50)
            without_abstract_count = len(deduplicated_citations) - with_abstract_count
            
            print(f"   - 去重后引用数据: {len(deduplicated_citations)} 篇")
            print(f"   - 有摘要: {with_abstract_count} 篇 ({with_abstract_count/len(deduplicated_citations)*100:.1f}%)")
            print(f"   - 无摘要: {without_abstract_count} 篇 ({without_abstract_count/len(deduplicated_citations)*100:.1f}%)")
            
            # 优先排序：有摘要的文献排在前面
            citations_with_abstract = [c for c in deduplicated_citations if c.get('abstract') and len(str(c.get('abstract', '')).strip()) > 50]
            citations_without_abstract = [c for c in deduplicated_citations if not (c.get('abstract') and len(str(c.get('abstract', '')).strip()) > 50)]
            
            # 按年份排序（优先新文献）
            def sort_by_year(citation):
                year_str = citation.get('year', '0')
                try:
            ...
            citations_with_abstract.sort(key=sort_by_year, reverse=True)
            citations_without_abstract.sort(key=sort_by_year, reverse=True)
            
            # 重新组合：有摘要的在前
            deduplicated_citations = citations_with_abstract + citations_without_abstract
            
            print(f"   ✅ 已按摘要优先级和年份重新排序")
            
            self.root.after(0, lambda: self.update_status(f"找到 {len(deduplicated_citations)} 个去重后的引用关系（{with_abstract_count}篇有摘要），开始智能筛选..."))
            ...
            print(f"      - 其中有摘要: {with_abstract_count} 篇")
            print(f"      - 参考高质量文章数: {len(high_quality_articles)}")
            print(f"      - 目标筛选数量: {target_count}")
            print(f"      💡 策略: 优先分析有摘要的文献，摘要长度扩展到2000字符")
            ...
            # 统计筛选结果中有摘要的比例
            filtered_with_abstract = sum(1 for c in filtered_citations if c.get('abstract') and len(str(c.get('abstract', '')).strip()) > 50)
            if filtered_citations:
                print(f"   📊 筛选结果统计:")
                print(f"      - 有摘要: {filtered_with_abstract} 篇 ({filtered_with_abstract/len(filtered_citations)*100:.1f}%)")
                print(f"      - 无摘要: {len(filtered_citations)-filtered_with_abstract} 篇")
            ...
            result_text += f"   其中有摘要: {with_abstract_count} 篇\n"
            result_text += f"   智能筛选后: {len(filtered_citations)}\n"
            result_text += f"   筛选结果有摘要: {filtered_with_abstract} 篇\n"
            ...
            for i, citation in enumerate(final_citations[:5], 1):
                has_abstract = "✅" if (citation.get('abstract') and len(str(citation.get('abstract', '')).strip()) > 50) else "❌"
                result_text += f"   {i}. {has_abstract} {citation.get('title', '未知标题')[:50]}...\n"
                result_text += f"      关系: {citation.get('relation', '未知')}\n"
                result_text += f"      年份: {citation.get('year', '未知')}\n"
                result_text += f"      期刊: {citation.get('journal', '未知')[:30]}\n"
    ...
    def _query_references_with_full_info(self, doi):
        """
        查询引用文献并为每个文献获取完整信息（包括摘要）
        这是关键改进：模仿测试代码的成功经验
        """
        import time
        
        print(f"      🔍 查询DOI [{doi}] 的引用文献基本信息...")
        ...
        try:
            ...
            if 'reference' not in message:
                print(f"      ❌ 该文献没有'reference'字段")
                return []
            
            references = message['reference']
            print(f"      ✅ 找到 {len(references)} 个引用文献基本信息")
            
            # 统计有DOI的引用文献
            refs_with_doi = [ref for ref in references if ref.get('DOI')]
            refs_without_doi = [ref for ref in references if not ref.get('DOI')]
            
            print(f"      📊 其中有DOI: {len(refs_with_doi)} 个，无DOI: {len(refs_without_doi)} 个")
            
            # 第二步：为有DOI的引用文献批量获取完整信息
            print(f"      🔄 开始为有DOI的引用文献获取完整信息（包括摘要）...")
            
            enhanced_references = []
            success_count = 0
            abstract_count = 0
            
            # 🔑 关键：只处理有DOI的前50个文献（避免API限流）
            max_enhanced = 50
            for i, ref in enumerate(refs_with_doi[:max_enhanced]):
                ref_doi = ref.get('DOI')
                ...
                # 获取完整信息
                full_info = self._get_full_article_info(ref_doi)
                ...
                if full_info:
                    ...
                    enhanced_ref = {
                        'doi': ref_doi,
                        'title': full_info.get('title', ref.get('article-title', '未知')),
                        'abstract': full_info.get('abstract', ''),  # 🔑 关键：摘要信息
                    ...
                    if enhanced_ref['abstract'] and len(str(enhanced_ref['abstract']).strip()) > 50:
                        abstract_count += 1
                    ...
                    enhanced_references.append({
                        'doi': ref_doi,
                        'title': ref.get('article-title', '未知'),
                        'abstract': '',
                        'year': ref.get('year', '未知'),
                        'authors': ref.get('author', '未知'),
                        'journal': ref.get('journal-title', '未知'),
                        'is_referenced_by_count': 0,
                        'type': '未知',
                        'url': f"https://doi.org/{ref_doi}",
                        'relation': 'referenced_by'
                    })
            
            # 第三步：添加无DOI的引用文献（只有基本信息）
            for ref in refs_without_doi[:20]:  # 最多添加20个无DOI的
                title = ''
            ...
            print(f"      ✅ 完整信息获取完成:")
            print(f"         - 处理有DOI文献: {min(max_enhanced, len(refs_with_doi))}")
            print(f"         - 成功获取完整信息: {success_count}")
            print(f"         - 其中有摘要: {abstract_count} 篇 ({abstract_count/success_count*100:.1f}%)" if success_count > 0 else "         - 其中有摘要: 0 篇")
            print(f"         - 添加无DOI文献: {min(20, len(refs_without_doi))}")
            print(f"         - 总计返回: {len(enhanced_references)} 篇")
    ...
    def _get_full_article_info(self, doi):
        """
        获取单篇文献的完整信息（包括摘要）
        这个函数模仿测试代码中的成功方法
        """
